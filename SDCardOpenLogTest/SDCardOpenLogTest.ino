...
{ 
  ...
  Serial2.begin(9600); //9600bps is default for OpenLog
  //Serial2.begin(57600); //Much faster serial, used for testing buffer overruns on OpenLog
  //Serial.begin(115200); //Much faster serial, used for testing buffer overruns on OpenLog
  Serial.begin(57600); //9600bps is default for OpenLog
  ...
  for(int numofTests = 0 ; numofTests < testAmt ; numofTests++)
  {
    ...
    for(int k = 33; k < 43 ; k++)
    {
      ...
      Serial2.write(k); //Print the ASCII value directly: ! then " then #, etc
  ...
  Serial2.print("Characters pushed: ");
  Serial2.println(totalCharacters);  
  Serial2.print("Time taken (s): ");
