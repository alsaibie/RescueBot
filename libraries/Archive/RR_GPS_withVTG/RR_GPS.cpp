...
int RR_GPS::readMessage(boolean readRMC)
{
	char RMCBuffer[100] = "", PLSRBuffer[70] = "", VTGBuffer[70] = "";
	boolean RMC_completed = false, PLSR_completed = false, VTG_completed = false;
	while(1)
	{
		if(Serial.available())
		{        
				...
				else if(_new_term)                            // $ sign not carried to buffer
				{
					...
					case _TERM_OTHER:                         //New Term. Store enough characters to determine if of interest.
						...
						{    
							...
							else if(strncmp(buffer,_VTG_ID,5)==0&&!VTG_completed)
							{
								
								if(readRMC){
									strcpy(VTGBuffer,buffer);
									buffer[0] = '\0';
									_term_type=_TERM_VTG;
								}
								else
								{
									_new_term=false;
					...
					case _TERM_RMC:
						...
						{    
							...
							bufferIndex=0;
							_new_term=false;
							Serial.println("RMC Reading completed");
							RMC_completed = true;
						}
						break;
					case _TERM_PLSR:
						...
						case _TERM_VTG:
						VTGBuffer[bufferIndex]=byteGPS;
						bufferIndex++;                                    
						if(byteGPS==10)
						{
							Serial.write(VTGBuffer);
							VTGBuffer[bufferIndex]='\0';
							bufferIndex=0;
							_new_term=false;
							Serial.println("VTG Reading completed");
							VTG_completed = true;
						}
						break;
					}
		...
		if((PLSR_completed && !readRMC)||RMC_completed && VTG_completed && PLSR_completed)
		{
			if(readRMC){
				extractData(RMCBuffer,RMC);
				extractData(PLSRBuffer,PLSR);
				extractData(VTGBuffer, VTG);
				if(status[0]!='A'){ 
					if(_gps_fix)
					{
						_gps_fix=false;
					...
					RMC_completed = false;
					VTG_completed = false;
					Serial.println("Invalid RMC data. Reading again!");
					break;
				}
				else
				{
...
void RR_GPS::storeVTGData(char value[], int segment)
{
	switch(segment)
	{
	case VTG_COURSE_TRUE:
		vtg_course_t=atof(value);
		break;
	case VTG_COURSE_MAG:
		vtg_course_m=atof(value);
		break;
	}
}
...
void RR_GPS::displayData(int type)
{
	...
	{
		Serial.println(F(" Printing Short VTG Report"));
		Serial.print(F(" True Course: "));
		Serial.println(vtg_course_t);
		Serial.print(F(" Magnetic Course: "));
		Serial.println(vtg_course_m);
...
void RR_GPS::updateTargetDistance()
{
  ...
  float bearing = fmod((atan2(y,x)*57.295779513+360),360); // Multiplied by constant to convert to decimal degrees.
  
  float _dir= (_gps_fix ? course : dir);
 
  Serial.println(bearing);
  Serial.println(_dir);
  delta_heading_reqd = bearing - _dir;
}
