...
#define INIT	F("$PSRF101,517781.3583,-528100.6426,3527350.9774,9600,586376,1755,12,3*24")
  ...
  #define VTG_COURSE_TRUE 1
  #define VTG_COURSE_MAG 3
  class RR_GPS{
  public:
    ...
    void Enable();
    void Disable();
    int readMessage(boolean readRMC);
    void displayData(int type);
    void updateTargetDistance(); // Return distance to target and change in heading required
	...
	//GPVTG Data
	float vtg_course_m, vtg_course_t;
  ...
  private:
    ...
    enum {_TERM_OTHER=0, _TERM_RMC=1, _TERM_PLSR=2, _TERM_VTG=3 };
    int _baud;
    int _enpin;
    byte _term_type;
	...
	void storeVTGData(char value[], int segment);
    bool _new_term;
